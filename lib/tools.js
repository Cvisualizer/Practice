...
function label(posX,posY,size,family,content,col){
	if(family=="") family="HelveticaNeue";
...
始点をfloat x1,y1で指定
終点をfloat y1,y2で指定
膨らみ具合をfloat sw(絶対値が1~9)で指定
*/
function drawArrow(x1,y1,x2,y2,sw,col){
	getArrow(x1,y1,x2,y2,sw,col);
}

function getArrow(x1,y1,x2,y2,sw,col){
	if(Math.abs(sw)<0) sw = 1;
	if(Math.abs(sw)>9) sw = 9;
	if(sw>0) sw = 10-sw;
	else sw = -10-sw
	...
	if(disAX>disAY){
		...
		//val="M"+x1+","+y1+"R"+(x1+disX/6)+","+(y1-disAX/sw)+" "+(x1+disX/3)+","+(y1-disAX/(sw*0.72))+" "+(x1+disX/2)+","+(y1-disAX/(sw*0.65))+" "+(x1+(disX/6)*4)+","+(y1-disAX/(sw*0.72))+" "+(x1+(disX/6)*5)+","+(y1-disAX/sw)+" "+x2+","+y2;
	}
	else{
	...
	path.id+="Path";
	path.transform=function(nx1,ny1,nx2,ny2,nsw,ms){
		if(nsw>0) nsw = 10-nsw;
		else nsw = -10-nsw
		disX=nx2-nx1;
		disY=ny2-ny1;
		disAX=Math.abs(disX);
		disAY=Math.abs(disY);
		if(disAX>disAY) val="M"+nx1+","+ny1+"R"+(nx1+disX/4)+","+(ny1-disAX/nsw)+" "+(nx1+disX/2)+","+(ny1-disAX/(nsw*0.75))+" "+(nx1+(disX/4)*3)+","+(ny1-disAX/nsw)+" "+nx2+","+ny2;
		else val="M"+nx1+","+ny1+"R"+(nx1-disAY/nsw)+","+(ny1+disY/4)+" "+(nx1-disAY/(nsw*0.75))+","+(ny1+disY/2)+" "+(nx1-disAY/nsw)+","+(ny1+(disY/4)*3)+" "+nx2+","+ny2;
		path.animate({
			d:val
		},ms)
	}
